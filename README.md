...
* Uses the Github provider to get a list of IP's where webhooks will be sent from Github. It then adds those to a firewall policy associatedwith the Web Application Firewall v2.
...
|------|-------------|------|---------|:--------:|
| <a name="input_atlantis_repo_whitelist"></a> [atlantis\_repo\_whitelist](#input\_atlantis\_repo\_whitelist) | the address for the github repo. do not include http://. just 'github.com/org/repo' is needed | `string` | `""` | no |
...
| <a name="input_atlantis_whitelist_ips"></a> [atlantis\_whitelist\_ips](#input\_atlantis\_whitelist\_ips) | We are protecting Atlantis with a WAFv2 App gateway. Git webhook IP's are being automatically added to the WAFv2 policy. Here you should include any public IP CIDR's you want to access the atlantis UI with . I.E. Home connection, Datacenters, offices, etc. | `list(string)` | <pre>[<br>  ""<br>]</pre> | no |
